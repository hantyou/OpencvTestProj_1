...
b = 0.0001
BinaryThreshold = 30
NumFrameForceForeToBack = 150 # 当一个目标多少帧之后，它会强制转换为背景
kernel1 = np.ones((5, 5), np.uint8)
kernel2 = np.ones((11, 11), np.uint8)
kernel3 = np.ones((3, 3), np.uint8)
kernel4 = np.ones((9, 9), np.uint8)
"""显示窗口初始化"""
# cv2.namedWindow("KNN Sub", cv2.WINDOW_KEEPRATIO)
# cv2.namedWindow("Color Subtracted", cv2.WINDOW_KEEPRATIO)
# cv2.namedWindow("Detect Mask", cv2.WINDOW_KEEPRATIO)
...
while 1:
    ...
    # ColorDist = (Sub[:, :, 0] + Sub[:, :, 1]  + Sub[:, :, 2] )/3
    # ret, SubShow = cv2.threshold(np.uint8(ColorDist),30, 255, type=cv2.THRESH_BINARY)
    ...
    # cv2.imshow("Color Subtracted", Show)
    # cv2.imshow("Detect Mask", subtract)
    # cv2.imshow("KNN Sub", subKNN)
